...
table_locks as (
        SELECT 
            distinct l.pid, c.relname AS table_name
        FROM 
            pg_locks l
        LEFT JOIN 
...
pid_table_locks as (
    select 
    pid, STRING_AGG(table_name, ', ') as locked_table_names 
    from table_locks
    group by pid
)
select 
    (select application_name from pg_stat_activity where pid = blocker_waiters.pid) as application_name,
    blocker_waiters.* ,
    pid_table_locks.locked_table_names
from 
    blocker_waiters
    left outer join pid_table_locks 
    on blocker_waiters.pid = pid_table_locks.pid
;
